...
spec:
  ...
  containers:
    ...
    - name: docker
      ...
      - "--insecure-registry=artifactory.apps.prod-nonpci-shared.nrapp.cloud"
      tty: true
      volumeMounts:
      - mountPath: /var/run/
        name: dind-socket
        ...
        stages {
          ...
          stage('Build') {
            ...
            steps {
              ...
              sed -i 's,https\\\\://services.gradle.org/distributions/,https://nexus.apps.prod-nonpci-shared.nrapp.cloud/repository/gradle-packages/,g' gradle/wrapper/gradle-wrapper.properties
              sed -i 's,https://artifactory.apps.dev-nonpci-shared.npnbank.local/artifactory/libs-release-local/,https://nexus.apps.prod-nonpci-shared.nrapp.cloud/repository/libs-release-local/,g' build.gradle
              ./gradlew assemble $GRADLE_BUILD_OPTIONS
              '''
          ...
          stage('Package') {
            ...
            steps {
              ...
              buildah login -u "$NEXUS_USR" -p "$NEXUS_PASSWORD" $NEXUS_DR_REPO
              buildah tag "${NEXUS_REPO}/${NAME}:${IMAGE_TAG}" "${NEXUS_DR_REPO}/${NAME}:${IMAGE_TAG}"
              buildah tag "${NEXUS_REPO}/${NAME}:${IMAGE_TAG}-${SHORT_COMMIT}" "${NEXUS_DR_REPO}/${NAME}:${IMAGE_TAG}-${SHORT_COMMIT}"
              buildah push "${NEXUS_DR_REPO}/${NAME}:${IMAGE_TAG}"
              buildah push "${NEXUS_DR_REPO}/${NAME}:${IMAGE_TAG}-${SHORT_COMMIT}"
              '''
            }
          }
          // when on release/prod, trigger a downstream deployment to development environment to keep it in line with HEAD
          // stage('Deploy Prod') {
          //   when {
          //     expression {
          //       if (params.BRANCH == 'release/1_0_2') {
          //         return true
          //       }
          //       return false
          //     }
          ...
          //   steps {
          //     script {
          //       build job: "/alpha/deploy-prod/$NAME", parameters: [string(name: 'BRANCH', value: params.BRANCH)]
          //     }                
          //   }
          // }
        }
...
//           }
//         }
//         // load credentials for the shared services cluster
//         environment {
...
//     enabled: true
// image:
//   repository: artifactory.apps.prod-nonpci-shared.nrapp.cloud/docker/${NAME}:${IMAGE_TAG}-${SHORT_COMMIT}
// host: ${NAME}.apps.${params.CROATIA_ENVIRONMENT}.nrapp.cloud
// hosts:
// - ${NAME}.apps.${params.CROATIA_ENVIRONMENT}.nrapp.cloud"""
...
//                     [
//                       'ref': 'values',
//                       'repoURL': "ssh://git@bitbucket.nrapp.cloud/alpha/${NAME}.git",
