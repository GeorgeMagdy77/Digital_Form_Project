      ...
      pipeline {
        ...
        stages {
          ...
          stage('Release') {
            when {
              expression {
                return GIT_BRANCH.contains('release/') || GIT_BRANCH == 'main' || GIT_BRANCH == 'origin/main'
              }
            }
            stages {
              stage('Setup') {
              ...
              stage('Deploy Development') {
                when {
                  expression {
                    return GIT_BRANCH == 'main' || GIT_BRANCH == 'origin/main'
                  }
                }
                steps {
                  script {
                    build job: "/alpha/deploy-development/$NAME", parameters: [string(name: 'BRANCH', value: GIT_BRANCH)]
                  }                
                }
              }
            }
