...
def call() {
  pipeline {
    ...
    environment {
      ...
      TARGET_SERVER = "https://api.${params.CROATIA_ENVIRONMENT}.npnbank.local:6443"
      ...
      stage('Add Role') {
        steps {
          ...
          curl -k -L \
            ...
            https://vault.apps.dev-nonpci-shared.npnbank.local/v1/sys/auth/${params.CROATIA_ENVIRONMENT}

          curl -k -L \
            --request POST \
            --header "X-Vault-Token: \$VAULT_TOKEN" \
            --data '@config.json' \
            https://vault.apps.dev-nonpci-shared.npnbank.local/v1/auth/${params.CROATIA_ENVIRONMENT}/config

          curl -k -L \
            --request PUT \
            --header "X-Vault-Token: \$VAULT_TOKEN" \
            --data '@policy.json' \
            https://vault.apps.dev-nonpci-shared.npnbank.local/v1/sys/policies/acl/${params.CROATIA_ENVIRONMENT}

          curl -k -L \
            --request POST \
            --header "X-Vault-Token: \$VAULT_TOKEN" \
            --data '@role.json' \
            https://vault.apps.dev-nonpci-shared.npnbank.local/v1/auth/${params.CROATIA_ENVIRONMENT}/role/alpha
          """
        }
      }
    }
