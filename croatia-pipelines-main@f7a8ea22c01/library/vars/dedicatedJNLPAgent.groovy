      ...
      pipeline {
        ...
        stages {
          ...
          stage('Build') {
            steps {
              sh '''
              sed -i 's,https\\\\://services.gradle.org/distributions/,https://artifactory.apps.dev-nonpci-shared.npnbank.local/artifactory/libs-release-local/gradle/gradle/gradle-packages/,g' gradle/wrapper/gradle-wrapper.properties
        ...
        stages {
          ...
          stage('Package') {
            ...
            steps {
              unstash 'build'
              sh '''
              docker login -u "$ARTIFACTORY_USR" -p "$DOCKER_TOKEN" artifactory.apps.dev-nonpci-shared.npnbank.local
              docker build -t "artifactory.apps.dev-nonpci-shared.npnbank.local/docker-local/${NAME}:${IMAGE_TAG}" .
              docker tag "artifactory.apps.dev-nonpci-shared.npnbank.local/docker-local/${NAME}:${IMAGE_TAG}" "artifactory.apps.dev-nonpci-shared.npnbank.local/docker-local/${NAME}:${IMAGE_TAG}-${SHORT_COMMIT}"
              docker push "artifactory.apps.dev-nonpci-shared.npnbank.local/docker-local/${NAME}:${IMAGE_TAG}"
              docker push "artifactory.apps.dev-nonpci-shared.npnbank.local/docker-local/${NAME}:${IMAGE_TAG}-${SHORT_COMMIT}"
              '''
            }
          }
...
gateway:
  ...
  tls:
    enabled: true
image:
  repository: artifactory.apps.dev-nonpci-shared.npnbank.local/docker/${NAME}:${IMAGE_TAG}-${SHORT_COMMIT}
